...
export const Jobs = () => {
  ...
  useEffect(() => {
    makeRequest({ url: '/jobs-review/' });
    console.log(jobs);
  }, []);

  useEffect(() => {
    if (success) {
      const pendingJobs = jobs.results.filter(
        (job) => job.status === 'Pending'
      );

      const approvedJobs = jobs.results.filter(
        (job) => job.status === 'Approved'
      );

      const declinedJobs = jobs.results.filter(
